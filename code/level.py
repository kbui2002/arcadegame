...
from sprites import Generic, Block, Animated, Particle, Coin, Player, Spikes, Hunter, Shell, Cloud
...
class Level:
	...
	def build_level(self, grid, asset_dict, jump_sound):
		for layer_name, layer in grid.items():
			for pos, data in layer.items():
				...
				match data:
					case 0: self.player = Player(pos, asset_dict['player'], self.all_sprites, self.collision_sprites, jump_sound,3)
					case 1: 
					...
					case 19: Coin('flag', asset_dict['flag'], pos, [self.all_sprites, self.coin_sprites])
					# enemies
					case 7: Spikes(asset_dict['spikes'], pos, [self.all_sprites, self.damage_sprites])
					case 8: 
						Hunter(asset_dict['hunter'], pos, [self.all_sprites, self.damage_sprites], self.collision_sprites)
					case 9: 
						Shell(
							orientation = 'left', 
	...
	def get_coins(self):
		...
		for sprite in collided_coins:
			if sprite.get_type() == "flag":	
				music = pygame.mixer.Sound("./audio/win.wav").play()
	...
	def get_damage(self):
		...
		if collision_sprites:
			self.hit_sound.play()
			heart = self.player.damage()
			if not heart:
				self.bg_music.stop()
				self.hit_sound.set_volume(0)
				self.loss = True
				self.all_sprites.loss()
				# self.switch()
	...
	def run(self, dt):
		...
		if self.loss:
				img = pygame.image.load("./graphics/menu/loss.png")
				self.display_surface.blit(img,(0,0))
		if self.win:
			img = pygame.image.load("./graphics/menu/win.png")
			self.display_surface.blit(img, (0,0))
...
class CameraGroup(pygame.sprite.Group):
	...
	def loss(self):
		...
		music = pygame.mixer.Sound("./audio/loss.wav")
