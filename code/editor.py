...
class Editor:
	def __init__(self, land_tiles, switch):
		...
		# music
		self.editor_music = pygame.mixer.Sound('./audio/Explorer.wav')
		self.editor_music.set_volume(0)
	...
	def event_loop(self):
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
				...
				self.switch(self.map)
				self.editor_music.stop()
			if event.type == pygame.MOUSEBUTTONDOWN and mouse_buttons()[0]:
				x = mouse_pos()[0]
				y = mouse_pos()[1]
				if 1187 <= x <= 1266 and 434 <= y <= 517:
					self.save(self.create_grid())
				if 1093 <= x <= 1176 and 438 <= y <= 517:
					self.map = self.load()
					self.switch(self.map)
	...
	def selection_hotkeys(self, event):
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_LEFT:
				self.selection_index -= 1
		self.selection_index = max(2,min(self.selection_index, 21))
	...
	def preview(self):
		...
		if not self.menu.rect.collidepoint(mouse_pos()):
			...
			elif self.selection_index != 20 and self.selection_index != 21:
	...
	def save(self,data):
		f = open(f"./level/level_{self.level + 1}.txt","w")
		f.write(str(data))
		f.close()
	
	def load(self):
		f = open(f"./level/level_{self.level}.txt","r")
...
class CanvasTile:
	...
	def add_id(self, tile_id, offset = vector()):
		...
		match options[tile_id]:
			...
			case "save"  : self.save = True
			case "load"  : self.load = True
			case _: # objects
	...
	def remove_id(self, tile_id):
		...
		match options[tile_id]:
			...
			case "save"  : self.save = False
			case "load"  : self.load = False
		self.check_content()

	def check_content(self):
		if not self.has_terrain and not self.has_water and not self.coin and not self.enemy and not self.save and not self.load:
			self.is_empty = True
